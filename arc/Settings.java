...
public class Settings {
	...
	/** Should print all edges in list? */
...
//	private String file2;	
	...
	public Settings(String[] args, Logger logger) throws ParseException {
		...
		if(line.hasOption(VERIFY_ALL)) {
			...
			this.verifyDirectory = line.getOptionValue(VERIFY_ALL);
		}
		else {
			this.verifyCurrentlyBlocked = 
					line.hasOption(VERIFY_CURRENTLY_BLOCKED);
			if(verifyCurrentlyBlocked) {
				this.verifyDirectory = line.getOptionValue(VERIFY_CURRENTLY_BLOCKED);
			}
			this.verifyAlwaysBlocked = line.hasOption(VERIFY_ALWAYS_BLOCKED);
			if(verifyAlwaysBlocked) {
				this.verifyDirectory = line.getOptionValue(VERIFY_ALWAYS_BLOCKED);
			}
			...
			if(verifyAlwaysIsolated) {
				this.verifyDirectory = line.getOptionValue(VERIFY_ALWAYS_ISOLATED);
			}
	...
	private Options getOptions() {
		...
		//additions by Stacia Fry for UCONN REU 2018
		option = new Option(REMOVE, true,
				"Remove node from network ETG");
		option.setArgName("NODE");
		options.addOption(option);
		...
		option = new Option(VERIFY_ALL, true,
				"Run all verifiers");
		option.setArgName("DIR/FILENAME");
		options.addOption(option);
		
		option = new Option(VERIFY_CURRENTLY_BLOCKED, true,
				"Verify currently blocked");
		option.setArgName("DIR/FILENAME");
		options.addOption(option);
		
		option = new Option(VERIFY_ALWAYS_BLOCKED, true,
		...
		option.setArgs(2);
		option.setArgName("K DIR/FILENAME");
		options.addOption(option);
		
		option = new Option(VERIFY_ALWAYS_ISOLATED, true,
	 ...
	 */
	public boolean shouldRemoveNode() {
		return (this.removeNode != null);
	}
	
	//additions by Stacia Fry for UCONN REU 2018
	/**
	 * Determine which node to be removed from ETG.
	 * @return the name of the node to be removed
	 */
